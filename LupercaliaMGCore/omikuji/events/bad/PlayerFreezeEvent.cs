...
namespace LupercaliaMGCore {
    public static partial class OmikujiEvents {        
        ...
        public static void playerFreezeEvent(CCSPlayerController client) {
            LupercaliaMGCore.getInstance().Logger.LogDebug("Player drew a omikuji and invoked Player freeze event");

            if(!client.PawnIsAlive) {
                LupercaliaMGCore.getInstance().Logger.LogDebug("Player freeze event failed due to player is alive. But this is should not be happened.");
                return;
            }
            ...
            if(playerPawn == null) {
                LupercaliaMGCore.getInstance().Logger.LogDebug("Player freeze event failed due to playerPawn is null.");
                return;
            }
            ...
            LupercaliaMGCore.getInstance().Logger.LogDebug("Player freeze event: Move type changed to MOVETYPE_OBSOLETE");
            ...
            LupercaliaMGCore.getInstance().AddTimer(PluginSettings.getInstance.m_CVOmikujiEventPlayerFreeze.Value, () => {
                ...
                LupercaliaMGCore.getInstance().Logger.LogDebug("Player freeze event: Move type changed to MOVETYPE_WALK");
            });
